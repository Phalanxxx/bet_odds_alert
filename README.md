...
khi tỉ lệ ≥ **60%** (ngưỡng trên) hoặc **tụt xuống dưới 57%** (ngưỡng dưới).
Chạy hoàn toàn trên cloud của GitHub — **không cần treo máy local**.
...
| Vừa **rớt khỏi mốc 60%** | ✅ `drop` | 60% → 59% |
| Vừa **tụt xuống dưới 57%** (ngưỡng dưới) | ✅ `fall` | 57% → 56% |
| Vẫn dưới 57% (đã báo `fall` rồi) | ❌ | 56% → 55% |
| Ở vùng giữa 57–60% | ❌ | 58% → 59% |

> `fall` chỉ báo **1 lần** khi vừa xuống dưới 57%; muốn báo lại thì tỉ lệ phải lên ≥ 57% rồi tụt xuống lại.
> Sau khi tụt xuống dưới 60% rồi vượt lên lại → tính là chu kỳ mới, `cross` bắn lại từ đầu.
> Nếu giá nhảy vọt xuyên cả 2 mốc trong 1 lần kiểm tra (vd 61% → 55%) → gửi **cả 2 email** `drop` + `fall`.
...
## Chỉnh sửa
- **Đổi ngưỡng trên:** sửa `ALERT_THRESHOLD` trong `.github/workflows/messi-monitor.yml`.
- **Đổi ngưỡng dưới:** sửa `ALERT_LOW_THRESHOLD` trong cùng file (đặt rỗng/xóa để tắt cảnh báo ngưỡng dưới).
